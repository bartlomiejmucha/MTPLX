...
At temperature 0 the two paths differ only where the model's own two best candidates are tied or one bf16 step apart, against median gaps of 2 to 11 nats over the agreed prefix. At the native sampler the speculative distribution sits at or below the autoregressive sample's own split-half distance at every joint length. A warm restore matches the cold state except where the model's two best candidates are an exact tie.
